...
class FakeLlmRequest:
    def __init__(self, response_name: str, response_payload: dict[str, Any]):
        self.contents = [
            types.Content(
                role="user",
                parts=[
...
def test_kagent_mcp_toolset_generates_unique_helper_names_per_server():
    first_agent = _make_agent_config("http://first.kagent:8080/mcp").to_agent("first_agent")
    second_agent = _make_agent_config("http://second.kagent:8080/mcp").to_agent("second_agent")
...
async def test_prompt_loader_adds_prompt_catalog_and_contents():
    ...
    llm_request = FakeLlmRequest(
        ...
        response_payload={"prompt_name": "incident_triage", "arguments": {"ticket_id": "INC-42"}},
    )
...
async def test_resource_loader_adds_resource_catalog_and_contents():
    ...
    llm_request = FakeLlmRequest(
        response_name=tool.name,
        response_payload={"resource_names": ["cluster_runbook"]},
    )
