...
class _BaseKAgentMcpLoaderTool(BaseTool):
    ...
    def _last_function_response(self, llm_request: "LlmRequest") -> dict[str, Any] | None:
        if not llm_request.contents or not llm_request.contents[-1].parts:
            return None

        function_response = llm_request.contents[-1].parts[0].function_response
        if function_response is None or function_response.name != self.name:
            return None

        return function_response.response or {}
...
class LoadKAgentMcpResourceTool(_BaseKAgentMcpLoaderTool):
    ...
    async def run_async(self, *, args: dict[str, Any], tool_context: "ToolContext") -> Any:
        resource_names = [str(name) for name in args.get("resource_names", []) if name]
        return {
    ...
    async def _append_selected_resources(self, *, tool_context: "ToolContext", llm_request: "LlmRequest") -> None:
        response = self._last_function_response(llm_request)
        if not response:
            return
        ...
        for resource_name in response.get("resource_names", []):
            try:
                ...
                logger.warning("Failed to read MCP resource '%s' from %s: %s", resource_name, self._server_label, error)
                continue
                    ...
                    parts=[
                        types.Part.from_text(
                            text=(
                                ...
                                f"returned message {index} with role '{role}':"
                            )
                        ),
