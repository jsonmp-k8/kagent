...
def main():
    ...
    if args.incremental and prev_graph is not None and changed_files:
        ...
        for edge_data in prev_edges:
            ...
            tgt_file = _edge_file(edge_data.get("target", ""))
            # Keep edges where both endpoints are from unchanged files
            if src_file not in changed_files and tgt_file not in changed_files:
